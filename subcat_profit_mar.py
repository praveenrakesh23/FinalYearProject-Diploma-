...
# Group by 'subcategory' and calculate the mean profit margin
subcategory_profit_margin = merged_df.groupby('subcategory')['ProfitMargin'].mean().reset_index()

# Sort the DataFrame by profit margin in descending order
subcategory_profit_margin = subcategory_profit_margin.sort_values(by='ProfitMargin', ascending=False)

# Close the database connection
conn.close()

# Create a bar chart for subcategory-wise profit margin
plt.figure(figsize=(12, 6))
plt.bar(subcategory_profit_margin['subcategory'], subcategory_profit_margin['ProfitMargin'], color= custom_colors)
plt.xlabel('Subcategory')
plt.ylabel('Profit Margin (%)')
plt.title('Subcategory-wise Profit Margin')
plt.tight_layout()
